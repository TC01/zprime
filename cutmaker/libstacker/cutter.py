...
def plotJetVar(name, jetvar, jetcut, exit, bins = 50, lower = 0, upper = 4000):
	...
	stack = libstacker.doPlotThings(name, cuts, vars, bins, lower, upper, location)
	if not exit:
		raw_input()
...
def main():
	...
	cuts = ["jet1csv > 0.697 || jet2csv > 0.697 || jet3csv > 0.697", "jet1csv < 0.55 || jet2csv < 0.55 || jet3csv < 0.55", "jet1csv < 0.75 || jet2csv < 0.75 || jet3csv < 0.75", "jet1csv < 0.5 || jet2csv < 0.5 || jet3csv < 0.5"]
	cutnames = ["Jets Btag", "Jets Toptag", "Jets Loose Wtag", "Jets Tight Wtag"]

	libstacker.doCutSequencing("test", cuts, cutnames, "UNMERGEDMASS", 50, 0, 4000, location)


def definitelyNotmain():
	global location
	...
	parser.add_option("-l", "--location", default=location, dest="location", help="The location to run over.")
	...
	if options.variable == "jetmass":
		plotJetVar("hadronic_" + options.jet + "_mass", "jet[X]mass", jetcut, options.exit)
