...
class multidist:
	...
	def __init__(self, n, b, s, d, bc, sc, mw):
		...
		self.bn = 0
		self.sn = 0
		self.bstack = THStack(n+"_bkg", "")
		self.sstack = THStack(n+"_sig", "")
	...
	def PLOTSTACKS(self, pn, bn, sn, dn, C, xname, logscale): # background/signal names (for legend). C are coordinates for the l. Save name for pdf is "pn".
			...
			hh = 0
			for i in self.b:
				hh = TMath.Max(hh, i.GetMaximum())
			for j in self.s:
				hh = TMath.Max(hh, j.GetMaximum())
			hh = TMath.Max(hh, self.sstack.GetMaximum())
			hh = TMath.Max(hh, self.bstack.GetMaximum())
